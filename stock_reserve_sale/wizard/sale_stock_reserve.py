...
class SaleStockReserve(models.TransientModel):
    ...
    def _default_location_id(self):
        ...
        if active_model == "sale.order" and (active_id or active_ids):
            order = self.env["sale.order"].browse(active_id or active_ids[0])
            warehouse = order.warehouse_id
        elif active_model == "sale.order.line" and active_ids:
            line = self.env["sale.order.line"].browse(active_ids[0])
